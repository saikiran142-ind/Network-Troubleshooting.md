...
3. Right-click your Wi-Fi driver (e.g., Intel/Realtek) and select **Uninstall device** (Do NOT delete driver software).
...
1. Run `ipconfig /release` and `ipconfig /renew` on the affected machine.
2. If it's a static IP, manually change the last digit of the IP address in **IPv4 Settings**.
...
- **Fix:** Verify if the **DNS Suffix** for the company domain is added in Advanced TCP/IP settings. Use `tracert` to identify if traffic is routing through the VPN tunnel or local ISP.
...
- **Fix:** 
  1. Enable Network Discovery and File/Printer Sharingin Advanced Sharing Settings.
  2. Ensure the "Function Discovery Resource Publication" service is running in `services.msc`.
